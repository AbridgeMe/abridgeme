...
- (id)initWithSidebarViewController:(GHRevealViewController *)sidebarVC 
					  withSearchBar:(UIImageView *)searchBar
...
- (void)viewDidLoad {
	...
	_menuTableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0f, 94.0f, kGHRevealSidebarWidth, CGRectGetHeight(self.view.bounds) - 44.0f)
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
	...
	if (headerText != [NSNull null]) {
		headerView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, [UIScreen mainScreen].bounds.size.height, 35.0f)];
		CAGradientLayer *gradient = [CAGradientLayer layer];
		gradient.frame = headerView.bounds;
		gradient.colors = @[
			(id)[UIColor colorWithWhite:0.2f alpha:1.0f],
			(id)[UIColor colorWithWhite:0.2f alpha:1.0f],
		];
		[headerView.layer insertSublayer:gradient atIndex:0];
		
		UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(20.0f,0.0f,65.0f, 25.0f)];
		textLabel.text = (NSString *) headerText;
		textLabel.font = [UIFont fontWithName:@"Avenir-Black" size:([UIFont systemFontSize] * 1.0f)];
